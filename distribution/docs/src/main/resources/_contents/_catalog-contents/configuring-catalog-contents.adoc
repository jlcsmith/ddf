...
|<<Catalog Ingest Duplication Plugin>>
|${ddf-branding-lowercase}.catalog.metacard.duplication.DuplicationPlugin
|Configure rules to check for duplicate data on ingest
...
|Metacard Attributes (duplicates fail ingest)
|failOnDuplicateList
|String  cardinality=1000
|A list of metacard attributes used in the duplication check against the local catalog.  If a duplicate is found, the ingest will fail.
|
|No

|Metacard attributes (duplicates cause a validation error)
|errorOnDuplicateList
|String  cardinality=1000
